...
class Reader {
    ...
    readMultipleBlocks(tag, firstDataBlock, noOfDataBlocks) {
        return new Promise ((resolve, reject) => {
            ...
            /* this.writeCommand(Commands.ISOStandardHostCommand, [ISOStandardCommands.ReadMultipleBlocks, 0x01, ]).then(response => {
    ...
    parseInventorySingle (data) {
        ...
        switch (type) {
            ...
            case TransponderType.ISO15693:
                ...
                const identifier = newBuffer.slice(1, 9)

                // Convert the transponder ID to a HEX string, the SDK does the same.
                const newTag = convertToHexString (identifier)
